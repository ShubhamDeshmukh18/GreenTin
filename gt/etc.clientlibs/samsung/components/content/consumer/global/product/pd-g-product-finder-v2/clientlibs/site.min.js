(() => {
  ...
  const selector = {
    ...
    visualLnb: '.nv14-visual-lnb__container',
    visualLnbNew: '.nv14-visual-lnb-new__container',
    explorerLnbNV07: '.nv07-explore-floating-navigation',
  };
  ...
  class Pd03ProductFinder {
    ...
    setElement() {
      ...
      this.el.lnbEl = [el.exploreEl, el.visualEl, el.exploreNV07El].find((el) => el.target[0]);
    }
    ...
    checkLnbOverlapCasesInMo() {
      const isNv14WithNv14New = el.visualEl && el.visualNewEl ? true : false;
    ...
    stickyHeader() {
      ...
      const withLnb = this.isExplorerLnb || this.isVisualLnb || this.isExplorerLnbNV07;
      if(!fixStart && withLnb) {
        ...
        if (BREAKPOINTS.MOBILE > winRect.width) {
          ...
          if(!withLnb) {
            ...
            if(!this.lnbAnimating) {
              ...
              this.el.lnbEl.off('transitionstart', this.handler.lnbTransitionStart).on('transitionstart', this.handler.lnbTransitionStart);
              this.el.lnbEl.off('transitionend', this.handler.lnbTransitionEnd).on('transitionend', this.handler.lnbTransitionEnd);
              ...
              if (currentScrollTop > this.lastScrollTop) {
                ...
                const lnbEls = [el.exploreEl, el.visualEl, el.exploreNV07El].filter((el) => el.target[0]);
                const stickyLnbsHeight = lnbEls.map((selector) => selector.target[0].offsetHeight);
                const currentStickyNavHeight = Math.max(...stickyLnbsHeight);
	...
	var buildProductInfoArea = function(product){
		...
		var buildPriceArea = function(product, upperStock, thirdPASeller) {
			...
			if(usePriceArea) {
				...
				if(siteCode === "cn") {
					topPrice += `<span class="quick-view__price-label">三星商城参考价格</span>`;
				}
				...
				if(useConfiguratorUrl) {
					...
					if(siteCode === "us" && ctaTypeUpperCase !== "PREORDER" &&
						...
						if(ctaTypeUpperCase === "WHERETOBUY" || ctaTypeUpperCase === "BACKORDER" || ctaTypeUpperCase === "BACKORDERED" || ctaTypeUpperCase === "INSTOCK" || ctaTypeUpperCase === "LOWSTOCK") {
							...
							// console.log("case 12");
							actionBtn += `
							<button class="cta cta--contained cta--black js-cta-stock" href="javascript:;" ${dataModelCodeAttr} ${dataModelNameAttr} ${dataShopSkuAttr} ${dataEppVirtualModelCodeAttr}
					...
					} else if((siteCode === "tr" || siteCode === "cn") && ctaTypeUpperCase === "WHERETOBUY") {
						// console.log("case 13");
						actionBtn += `
		...
		$ctaBtn.on("click.finder", function(e){
			...
			if(isGPv2){
				...
				if(configInfo !== ""){
					...
					//new-hybris
					if(isNewHybris && siteCode ==='uk'){ //20210402 SEBN old hybris -- 20210408 id/ph //uk
		...
		var buildProductCardV2 = function(product) {
			...
			var productCardPriceAreaBuild = function(product, upperStock, thirdPASeller, cardLayout) {
				...
				if(usePriceArea) {
					...
					if(siteCode === "cn") {
						topPrice += `<span class="pd03-product-card__price-label">三星商城参考价格</span>`;
					}
...
//				} else 
				...
				//	`;
				//} else 
				if((reviewUseYN === "Y" || reevooUseYN === "Y") && isNotNull(frontModel.ratingHtml)) {
					if(!(!isNonShop && frontModel.ctaTypeUpperCase === "LEARNMORE")
...
//				}
			...
			var productCardCTABuild = function(product, originPrice, discountPrice, dataPricetext, dataMonth) {
				...
				if(!frontModel.isDeSkuExpModel){
					if(useConfiguratorUrl) {
						...
						if(siteCode === "us" && ctaTypeUpperCase !== "PREORDER" &&
							...
							if(ctaTypeUpperCase === "WHERETOBUY" || ctaTypeUpperCase === "BACKORDER" || ctaTypeUpperCase === "BACKORDERED" || ctaTypeUpperCase === "INSTOCK" || ctaTypeUpperCase === "LOWSTOCK") {
								...
								// console.log("case 12");
								actionBtn += `
								<button class="cta cta--contained cta--black ${stockCss}" href="javascript:;" ${dataModelCodeAttr} ${dataModelNameAttr} ${dataShopSkuAttr} ${dataEppVirtualModelCodeAttr}
					...
					} else if(siteCode == "cn" && isNotNull(frontModel.stockStatusText) && frontModel.stockStatusText == "inStock" && ctaTypeUpperCase == "WHERETOBUY") {
						actionBtn += `
						<button class="cta cta--contained cta--black js-pfv2-buy-now" href="${frontModel.shopCtaUrl}" data-link_info="${frontModel.shopCtaUrl}" data-config_info="" ${taggingAttrForBuy} ${buyNowtoLinkTaggingAttr}>
						${buyNowCta}
						</button>
		...
		var getProductModelInfo = function(product, frontModelIdx){
			...
			if(isNotNull(frontModelIdx) && isNotNull(tempProduct.modelList) && isNotNull(tempProduct.modelList[frontModelIdx])){
				...
				if(isNewHybris && !(siteCode === "pe" || siteCode === "cl" || siteCode === "mx" || siteCode === "co")){
