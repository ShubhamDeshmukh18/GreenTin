...
(function ($) {
	...
	const calculateInstallmentList = ["uk", "ee", "lv", "lt", "pl", "hu", "es", "ro", "ua","nz","sg","my","ph","vn","th","sk","cz"]; 
	...
	var emiDescEachSiteCode = function(siteCode, code, descriptionText){
		...
		} else if(siteCode == "ee" || siteCode ==  "lv" || siteCode ==  "lt" || siteCode == "pl" || siteCode == "hu" || siteCode == "ro" || siteCode == "ua" || siteCode == "nz" || siteCode == "sg" || siteCode == "my" || siteCode == "ph" || siteCode == "vn" || siteCode == "sk" || siteCode == "cz"){
			descriptionHtml +=  '<div class="finance-popup__description">'+
									'<p>'+
										descriptionText+
									'</p>'+
		...
		for(var idx=0; idx<values.length; idx++){
			...
			if(siteCode === "uk" && isB2B){
				...
				contentHtml += 	'<div class="finance-popup__additional">'+
									...
									'<div class="finance-popup__additional-content">'+
										...
										'<p>Spread the cost over <strong>12</strong> to <strong>24 months</strong> at <strong>0% APR</strong> representative, or 36 months at 14.9% APR representative.</p>'+
			...
			if(tabData.values && tabData.values.length > 0){
				...
				for(var jdx=0; jdx<tabData.values.length; jdx++){
					...
					if(siteCode == "uk" || siteCode == "ro" || siteCode == "ua" || siteCode == "nz" || siteCode == "sg" || siteCode == "my" || siteCode == "ph" || siteCode == "vn"){
...
(function ($) {
    ...
    var openSuccess = function () {
        var successText = Granite.I18n.get("We will email you when inventory is added.");
...
(function (win, $) {
	...
	function changePopup(wishlistCategory) {
		...
		aTagBlack.setAttribute("class","cta cta--outlined cta--black");
		...
		aTagEmphasis.setAttribute("class","cta cta--contained cta--emphasis");
	...
	function changePopupEvent(wishlistCategory, wishareaTarget) {
		...
		if(shopIntegrationFlag === 'GPv2' || isNewHybris || isB2B){				//new-hybris	
			$(document).off("click", ".wishlist-popup .wishlist-popup__cta-wrap .cta--emphasis");
			$(document).on("click", ".wishlist-popup .wishlist-popup__cta-wrap .cta--black", function () {
				wishlistPopup.hide();
			...
			if(wishlistCategory === 'notForSale' || wishlistCategory === 'completeWishlist'){
				$(document).on("click", ".wishlist-popup .wishlist-popup__cta-wrap .cta--emphasis", function () {
					wishlistPopup.hide();
					window.sg.components.wishPopup.closePopup();
					window.sg.components.wishlistIcon.closeWishlistPop(wishareaTarget);
				});
			...
			$(document).off("click", ".wishlist-popup .wishlist-popup__cta-wrap .cta--emphasis");
			$(document).off("click", ".wishlist-popup .wishlist-popup__cta-wrap .cta--black");
			$(document).on("click", ".wishlist-popup .wishlist-popup__cta-wrap .cta--black", function () {
				wishlistPopup.hide();
				window.sg.components.wishlistIcon.closeWishlistPop(wishareaTarget);
			});
							...
							productCardTemplate +=
								'<div class="compare-popup__price-current"'+priceTextForCompareAttr+'>'+
									...
									'<span class="compare-popup__price-current-text">'+
										Granite.I18n.get("From {0}/mo",[monthlyPrice])+
										' ';
										if(leasingInterest !== "" && siteCode !== "de"){
											productCardTemplate += 
												Granite.I18n.get("for {0} mos at {1}% APR",[tenureVal, leasingInterest]);
										} else {
											productCardTemplate += 
	...
	function buildProductCard(product) {
		...
		let productCardTemplate = "";
			...
			'<div class="compare-popup__product-card">'+
				...
				if((clReviewUseYN === "Y" || clReevooUseYN === "Y") &&isNotNull(model.ratingHtml) /* && !(clSiteCode === "br" && productGroupCode === GROUPCODE_IM) */){
					let ratingEmptyCls = "";
			...
			if(!priceDisplayNull){
				if(clSiteCode === "us" && ctaTypeUpperCase !== "PREORDER" && 
					...
					if(ctaTypeUpperCase === "WHERETOBUY" || ctaTypeUpperCase === "BACKORDER" || ctaTypeUpperCase === "BACKORDERED" || ctaTypeUpperCase === "INSTOCK" || ctaTypeUpperCase === "LOWSTOCK"){
						...
						tempProductCardTemplate +=
							'<button class="cta cta--contained cta--black cta--2line-mo cta--dense js-cta-stock" type="button" aria-label="'+Granite.I18n.get("Get stock alert")+' : '+displayName+'"'+dataModelCodeAttr + dataModelNameAttr+dataEppVirtualModelCodeAttr+
			...
			if(clIsGPv2){
				...
				if(configInfo !== ""){
					...
					if(isB2b){
						...
						if(isNewHybris && clSiteCode ==='uk'){ //202010402 SEBN old hybris 적용, 20210408 id/ph //uk 적용
